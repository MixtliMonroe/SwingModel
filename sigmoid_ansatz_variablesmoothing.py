...
def generate_r(l, g, delta, rdot_max):
  ...
  def k(u,v):
    return np.where(np.abs(u)*thetadot_normalisation_factor - np.abs(v) < 0,
                    4*rdot_max/(delta*v**2),
                    (4*(l - delta/2)*rdot_max)/(delta*g*u*np.sin(u)))
...
if __name__ == "__main__":
  ...
  r = generate_r(l=l, g=g, delta=delta, rdot_max=rdot_max)
  Model.set_r(r=r)

  # Solve using solve_ivp
  sol = Model.solve(t_eval = np.arange(0, 50, 1e-3), q0 = [np.pi/4, 0])
  theta = sol.y[0, :]
  theta_dot = sol.y[1, :]
  t = sol.t
  ...
  if False: # Plot drdt
    ...
    ax.set_ylim(-1.1*rdot_max, 1.1*rdot_max)
    ax.set_yticks(np.array(range(-2,3,1))*rdot_max, [str(i) for i in range(-2,3,1)])
    ax.set_ylabel(r"$\dot{r}/\dot{r}_{\text{max}}$")
